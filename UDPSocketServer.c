...
int main( int argc, char *argv[] )
{
    ...
    if(argc == 6)
    {
        ...
        {
            struct Peer newPeer;
            strcpy(newPeer.peer_name,argv[2]);
            strcpy(newPeer.ip_address_str, argv[3]);
            newPeer.m_port = atoi(argv[4]);
            newPeer.p_port = atoi(argv[5]);
            printf("Successfully Register: \n Peer: %s\n IP Address: %s\n Manager Port: %d \n Peer Port: %d",newPeer.peer_name,
            newPeer.ip_address_str,newPeer.m_port,newPeer.p_port);
        }
        else
        {
