...
void setRandPeerToDHT(int leader_index, int n)
{
    ...
    while(selected_count < (n-1))
    {
        ...
        if(random_index != leader_index && strcmp(registered_peers[random_index].state, "Free") == 0 || random_index == 1) {
...
int main( int argc, char *argv[] )
{
    ...
    } else if(strncmp(echoBuffer,"setup-dht",strlen("setup-dht")) == 0) 
    {
        ...
        if((strcmp(registered_peers[1].state,"Leader") != 0))
        {
        ...
        {
            strcpy(registered_peers[0].state, "InDHT");
        }
        ...
        registered_peers[leaderIndex].state);
        ...
        for(int i =0; i < num_registered_peers;i++)
        {
            ...
            sprintf(response + strlen(response), "%s %s %u %s\n", registered_peers[i].name,
            registered_peers[i].ip_address,registered_peers[i].p_port, registered_peers[i].state);
        }
    ...
    else if(strncmp(echoBuffer,"Deregistered",strlen("Deregistered")) == 0){
        char peer_name[MAX_NAME_LENGTH+1];
        sscanf(echoBuffer, "Deregistered %s",  peer_name);
