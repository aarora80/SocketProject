...
void insertEnd(struct Node** headRef, struct PeerTuple newTuple) {
    printf("I entered insertEND\n");
    struct Node* newNode = createNode(newTuple);
    if (*headRef == NULL) {
        *headRef = newNode;
        printf("HEAD IS NULL BRO");
        return;
    }
...
int main( int argc, char *argv[] )
{
    ...
    while(1){
            ...
            */
            ...
            if(strncmp(message, "Teardown", strlen("Teardown")) == 0)
            {
                ...
                if(strcmp(peer.name,"B") == 0)
                {
                  ...
                  sprintf(message,"setup-dht %s %d %d", "B", 4, 1996);
                    if (sendto(sock, message, strlen(message), 0, (struct sockaddr *)&echoServAddr, sizeof(echoServAddr)) < 0) {
                            perror("sendto failed");
                            exit(1);
                    }
        ...
        else if(strcmp(command, "rebuild-dht") ==0)
        {
            /*
            Command to set id of peers
            None leader peer enters this command to wait for set-id message from Leader
            */
            ...
            if(strncmp(message, "Teardown", strlen("Teardown")) == 0)
            {
                ...
                {
                  char message2[MAX_MESSAGE_SIZE];
                  sprintf(message,"setup-dht %s %d %d", "B", 5, 1996);
                    if (sendto(sock, message, strlen(message), 0, (struct sockaddr *)&echoServAddr, sizeof(echoServAddr)) < 0) {
                            perror("sendto failed");
                            exit(1);
                    }
        ...
        else if(strcmp(command, "join-dht") == 0)
        {
            printf("Rebuilding DHT\n");
            printf("Teardown Complete sent to manager!!!\n");
            char message[MAX_MSG_LENGTH];
            deleteHashTable();
            sprintf(message, "Teardown Complete Join!!!\n");
            if (sendto(sock, message, strlen(message), 0, (struct sockaddr *)&echoServAddr, 
            sizeof(echoServAddr)) < 0) 
            {
                    perror("sendto failed");
        ...
        else if (strcmp(command, "de-register") == 0){
            char message[MAX_MESSAGE_SIZE];

            sprintf(message,"Deregistered %s", peer.name);
...
//------------------Parsing Data File-------------------------------
        ...
        if(strcmp(peer.name,"B")==0 && check !=3){
            n = 4;
            check =3;
        }
        else
        ...
        while (fgets(line, sizeof(line), file) != NULL) {
            ...
            sscanf(line, "%lu,%99[^,],%d,%19[^,],%99[^,],%c,%99[^,],%d,%d,%d,%d,%19[^,],%d",
                    &event.event_id, event.state, &event.year, event.month,
                    event.event_type, &event.cz_type, event.cz_name,
                    &event.injuries_direct, &event.injuries_indirect,
                    &event.deaths_direct, &event.deaths_indirect,
